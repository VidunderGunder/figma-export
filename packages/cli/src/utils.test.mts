...
describe('Utils', () => {
    describe('requirePackages', () => {
        it('should require a package given a relative path', async () => {
            expect(await Promise.all(requirePackages([]))).to.deep.equal([]);

            expect(await Promise.all(requirePackages(['./utils.requirePackages.mock.cjs'], { param: 1 }))).to.deep.equal([{ param: 1 }]);
        });
    });
